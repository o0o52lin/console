...
abstract class Base
{
     ...
     * @return boolean
     */
    protected function checkMqer($taskid)
    {
        //处理的业务名称，父类类名
        $type = get_called_class();
        
        $this->log('开始处理业务:' . $type . ',任务编号:' . $taskid);
        // 查询 任务
        $timer = $this->db('uc_master')->select('id')->from('yz_mq')->where('`id`=' . $taskid . ' AND `consumer`= 3 AND `state`= 0')->limit(1)->row();
        if(!is_array($timer) or empty($timer)){
            unset($timer);
            return $this->error(1000, 'yz_mq定时任务(' . $taskid . ')不存在或已完成|关闭');
        }
        // 获取执行锁
        $lock = $this->db('uc_master')->update('yz_mq')->set('state',1)->where('`id`=' . $taskid . ' AND `consumer`= 3 AND `state`= 0')->query();
        if($lock !== 1){
            return $this->error(1000, 'yz_mq定时任务(' . $taskid . ')获取执行锁失败' . $this->db('uc_master')->lastSQL() . $this->db('uc_master')->getError());
        }
        return true;
    }
     ...
     * @return boolean
     */
    protected function delMqer($taskid)
    {
        $taskid = intval($taskid);
        if( $taskid <= 0 )
        {
        ...
        $rs = $this->db('uc_master')->delete('yz_mq')->where('`id`='.$taskid)->query();
        if($rs)
        {
            return true;
        }
     ...
     * @return boolean
     */
    protected function reRunMqer($taskid)
    {
        $taskid = intval($taskid);
        if( $taskid <= 0 )
        {
        ...
        $rs = $this->db('uc_master')->update('yz_mq')->set('state',0)->where('`id`='.$taskid)->query();
        if($rs)
        {
            return true;
        }
     ...
     * @return boolean
     */
    protected function runErrorMqer($taskid)
    {
        $taskid = intval($taskid);
        if( $taskid <= 0 )
        {
        ...
        $rs = $this->db('uc_master')->update('yz_mq')->set('state',3)->where('`id`='.$taskid)->query();
        if($rs)
        {
            return true;
        }
     ...
     * @return boolean
     */
    protected function runSuccessMqer($taskid)
    {
        $taskid = intval($taskid);
        if( $taskid <= 0 )
        {
        ...
        $rs = $this->db('uc_master')->update('yz_mq')->set('state',2)->where('`id`='.$taskid)->query();
        if($rs)
        {
            return true;
        }
