...
class DbConnection 
{
    ...
    * @return int
    */
    public function count($query = '',$params = null, $fetchmode = \PDO::FETCH_ASSOC)
    {
        $query = trim($query);
        if(empty($query))
        {
        ...
        return intval(isset($ret['ct']) ? $ret['ct'] : 0);
    }
    ...
    */
	 ...
	 * 遇到一个bug，原因未详
	 * 
	 * 程序数据库的第一个执行sql应避免先执行$this->db->beginTransaction();
	 * 不然有一定的几率会出现以下sql错误
	 * ERROR[HY000] SQLSTATE[HY000]: General error: 2014 Cannot execute queries while other unbuffered queries are active. 
	 ...
	 * 
	 * 感觉并不是错误提示的原因，可能是同时执行了两个sql，前一个sql没执行完，后一个就执行了，会出现上面的错误
	 * 
	 * 在本地环境正常，在线上有一定的几率出现上面的错误
	 * 引起sql错误原因，暂时不懂为什么，不让$this->db->beginTransaction();作为第一个sql执行，错误消失
     */
    public function beginTransaction()
    {
    	if(!$this->transCounter++){
    	    //解决db长时间空闲自动断开,报ERROR[HY000] SQLSTATE[HY000]: General error: 2006 MySQL server has gone away的问题
    	    try {
    	        if(!$this->pdo){
    	            $this->connect();
    	        }
